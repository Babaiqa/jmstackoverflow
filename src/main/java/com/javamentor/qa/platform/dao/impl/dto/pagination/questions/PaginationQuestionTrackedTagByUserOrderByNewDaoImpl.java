...
@Repository(value = "paginationQuestionTrackedTagByUserOrderByNew")
public class PaginationQuestionTrackedTagByUserOrderByNewDaoImpl implements PaginationDao<QuestionDto> {
    @PersistenceContext
    ...
    public int getCount(Map<String, Object> parameters) {
        return (int) (long) em.createQuery("select count(q) from Question q join q.tags tag join TrackedTag tt on tag.id = tt.trackedTag.id where tt.user in :id")
